...
export async function generateOfficialAfd(registrarId: string, startDate: string, endDate: string, generatedBy: string) {
  ...
  try {
    ...
    const fileName = afdFileName(source.registrar.inpiRegistration!, source.employer.id); const hash = createHash("sha256").update(artifact.bytes).digest("hex"); const year = startDate.slice(0, 4); const key = `official-documents/afd/${source.establishment.id}/${year}/${generation.id}/${fileName}`;
...
export async function downloadSignedAfd(id: string, userId: string) { const row = await getAfdGeneration(id); if (!row?.generation.fileKey || row.generation.status !== "SIGNED" || row.generation.signatureStatus !== "SIGNED" || !row.generation.fileHash || !row.generation.signatureFileKey) return undefined; const [bytes, signature] = await Promise.all([getPrivateObjectBytes(row.generation.fileKey), getPrivateObjectBytes(row.generation.signatureFileKey)]); if (createHash("sha256").update(bytes).digest("hex") !== row.generation.fileHash || createHash("sha256").update(signature).digest("hex") !== row.generation.signatureHash) throw new Error("AFD_INTEGRITY_MISMATCH"); await recordAudit(db, { action: "AFD_DOWNLOADED", entity: "AFD_GENERATION", entityId: id, performedBy: userId, after: { fileHash: row.generation.fileHash } }); return createPrivateDownloadUrl(row.generation.fileKey, row.generation.fileName ?? "AFD.txt"); }
