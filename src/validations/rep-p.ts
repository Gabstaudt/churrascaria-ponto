...
export const establishmentCreateSchema = z.object({
  ...
  cnpj: z.string().transform((value) => value.replace(/\D/g, "")).pipe(z.string().length(14, "Informe um CNPJ válido com 14 dígitos.")),
  registrarName: z.string().trim().min(2, "Informe o nome do registrador.").max(150),
  registrarIdentifier: z.string().trim().min(2, "Informe um identificador para o registrador.").max(100),
});
...
export const terminalCreateSchema = z.object({ name: z.string().trim().min(2, "Informe um nome para o terminal.").max(150), deviceIdentifier: z.string().trim().min(2, "Informe um identificador para o dispositivo.").max(150) });
