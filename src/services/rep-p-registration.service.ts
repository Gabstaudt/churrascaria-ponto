...
export type RegisterRepPPointInput = { employeeId: string; collectorId: string; eventType: "CLOCK_IN" | "CLOCK_OUT"; idempotencyKey: string };
...
export async function registerRepPPoint(input: RegisterRepPPointInput, clock: OfficialClock = systemOfficialClock) {
  const [existing] = await db.select().from(timeEntries).where(and(eq(timeEntries.collectorId, input.collectorId), eq(timeEntries.idempotencyKey, input.idempotencyKey))).limit(1); if (existing) return registrationResult(existing, input.eventType, true);
  ...
  try { return await db.transaction(async (tx) => { const nsr = await nextRepPNsr(tx, context.establishmentId); const [entry] = await tx.insert(timeEntries).values({ employeeId: employee.id, occurredAt, source: "REP_P", externalId: `rep-p:${context.registrarId}:${nsr}`, deviceIdentifier: context.deviceIdentifier, establishmentId: context.establishmentId, registrarId: context.registrarId, collectorId: context.collectorId, nsr, idempotencyKey: input.idempotencyKey, metadata: { eventType: input.eventType, timezone: context.timezone, original: true } }).onConflictDoNothing({ target: [timeEntries.collectorId, timeEntries.idempotencyKey] }).returning(); if (!entry) throw new ConcurrentRepPReplay(); return registrationResult(entry, input.eventType, false); }); } catch (error) { if (!(error instanceof ConcurrentRepPReplay)) throw error; const [replayed] = await db.select().from(timeEntries).where(and(eq(timeEntries.collectorId, input.collectorId), eq(timeEntries.idempotencyKey, input.idempotencyKey))).limit(1); if (!replayed) throw new Error("Reenvio concorrente não localizado."); return registrationResult(replayed, input.eventType, true); }
}
