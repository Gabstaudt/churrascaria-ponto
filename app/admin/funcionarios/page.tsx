  ...
  return (
    <main className="dashboard-page">
      ...
      <section className="data-panel" aria-label="Lista de funcionários">
        ...
        {result.items.length ? (
          <>
            <div className="table-scroll desktop-employee-list"><table className="data-table"><thead><tr><th>Funcionário</th><th>Matrícula</th><th>Cargo</th><th>Admissão</th><th>Status</th></tr></thead><tbody>{result.items.map((employee) => <tr key={employee.id}><td><Link className="employee-identity employee-link" href={`/admin/funcionarios/${employee.id}`}><span className="employee-initials">{employee.fullName.split(" ").slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span><span><strong>{employee.fullName}</strong><small>{formatCpf(employee.cpf)}</small></span></Link></td><td>{employee.registrationNumber}</td><td>{employee.position}</td><td>{formatDate(employee.admissionDate)}</td><td><EmployeeStatusBadge status={employee.status} /></td></tr>)}</tbody></table></div>
            <div className="mobile-employee-list">{result.items.map((employee) => <Link className="employee-card" href={`/admin/funcionarios/${employee.id}`} key={employee.id}><header><div className="employee-identity"><span className="employee-initials">{employee.fullName.split(" ").slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span><span><strong>{employee.fullName}</strong><small>{formatCpf(employee.cpf)}</small></span></div><EmployeeStatusBadge status={employee.status} /></header><dl><div><dt><Hash size={15} /> Matrícula</dt><dd>{employee.registrationNumber}</dd></div><div><dt><BriefcaseBusiness size={15} /> Cargo</dt><dd>{employee.position}</dd></div><div><dt><CalendarDays size={15} /> Admissão</dt><dd>{formatDate(employee.admissionDate)}</dd></div></dl></Link>)}</div>
          </>
        ) : (
