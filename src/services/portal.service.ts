...
export async function managedEmployeeIds(managerUserId: string) { return (await db.select({ id: managerEmployees.employeeId }).from(managerEmployees).where(eq(managerEmployees.managerUserId, managerUserId))).map((item) => item.id); }
export async function listManagerRequests(managerUserId: string) { const ids = await managedEmployeeIds(managerUserId); if (!ids.length) return []; return db.select({ id: correctionRequests.id, employeeId: correctionRequests.employeeId, employeeName: employees.fullName, date: correctionRequests.date, type: correctionRequests.type, requestedTime: correctionRequests.requestedTime, reason: correctionRequests.reason, status: correctionRequests.status, createdAt: correctionRequests.createdAt }).from(correctionRequests).innerJoin(employees, eq(employees.id, correctionRequests.employeeId)).where(inArray(correctionRequests.employeeId, ids)).orderBy(desc(correctionRequests.createdAt)); }
export async function listManagerSwaps(managerUserId: string) { const ids = await managedEmployeeIds(managerUserId); if (!ids.length) return []; return db.select({ id: dayOffSwaps.id, employeeId: dayOffSwaps.employeeId, employeeName: employees.fullName, dayOffDate: dayOffSwaps.dayOffDate, workDate: dayOffSwaps.workDate, reason: dayOffSwaps.reason, status: dayOffSwaps.status, requestedBy: dayOffSwaps.requestedBy }).from(dayOffSwaps).innerJoin(employees, eq(employees.id, dayOffSwaps.employeeId)).where(inArray(dayOffSwaps.employeeId, ids)).orderBy(desc(dayOffSwaps.createdAt)); }
export async function reviewManagerSwap(id: string, managerUserId: string, decision: "APPROVED" | "REJECTED", reason: string) { const scope = await managedEmployeeIds(managerUserId); const [current] = await db.select().from(dayOffSwaps).where(eq(dayOffSwaps.id, id)).limit(1); if (!current || !scope.includes(current.employeeId)) return undefined; const saved = await reviewDayOffSwap(id, decision, reason, managerUserId); if (saved) await db.insert(notifications).values({ recipientUserId: current.requestedBy, title: decision === "APPROVED" ? "Troca aprovada" : "Troca rejeitada", message: reason, link: "/portal/solicitacoes" }); return saved; }
export async function reviewCorrectionRequest(id: string, managerUserId: string, decision: "APPROVED" | "REJECTED", reason: string) {
  const scope = await managedEmployeeIds(managerUserId); const [current] = await db.select().from(correctionRequests).where(and(eq(correctionRequests.id, id), eq(correctionRequests.status, "PENDING"))).limit(1); if (!current || !scope.includes(current.employeeId)) return undefined;
