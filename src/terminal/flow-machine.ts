...
export type TerminalFlowState = { status: TerminalFlowStatus; locationValidationId?: string; biometricValidationId?: string; attemptId?: string; contingencyTicket?: string; employee?: IdentifiedEmployee; result?: { eventLabel: string; recordedAt: string; nsr: string }; error?: string };
export type TerminalFlowEvent = { type: "START" } | { type: "LOCATION_CAPTURED" } | { type: "LOCATION_VALID"; validationId: string } | { type: "LOCATION_REJECTED"; message: string } | { type: "FACE_CAPTURED" } | { type: "BIOMETRIC_APPROVED"; employee: IdentifiedEmployee; biometricValidationId: string; attemptId: string; contingencyTicket: string } | { type: "BIOMETRIC_REJECTED"; message: string } | { type: "RETRY_FACE" } | { type: "IDENTIFIED"; employee: IdentifiedEmployee } | { type: "VALIDATED" } | { type: "REGISTER" } | { type: "SUCCEEDED"; result: NonNullable<TerminalFlowState["result"]> } | { type: "FAILED"; message: string } | { type: "RESET" };
...
export function terminalFlowReducer(state: TerminalFlowState, event: TerminalFlowEvent): TerminalFlowState {
  ...
  switch (state.status) {
    ...
    case "REQUESTING_LOCATION": return event.type === "LOCATION_CAPTURED" ? { status: "CHECKING_LOCATION" } : event.type === "LOCATION_REJECTED" ? { status: "LOCATION_REJECTED", error: event.message } : state;
    case "CHECKING_LOCATION": return event.type === "LOCATION_VALID" ? { status: "WAITING_FOR_FACE", locationValidationId: event.validationId } : event.type === "LOCATION_REJECTED" ? { status: "LOCATION_REJECTED", error: event.message } : state;
    case "WAITING_FOR_FACE": return event.type === "FACE_CAPTURED" ? { ...state, status: "CHECKING_FACE" } : event.type === "BIOMETRIC_REJECTED" ? { ...state, status: "IDENTIFICATION_FAILED", error: event.message } : state;
    case "CHECKING_FACE": return event.type === "BIOMETRIC_APPROVED" ? { ...state, status: "VALIDATING", employee: event.employee, biometricValidationId: event.biometricValidationId, attemptId: event.attemptId, contingencyTicket: event.contingencyTicket } : event.type === "BIOMETRIC_REJECTED" ? { ...state, status: "IDENTIFICATION_FAILED", error: event.message } : state;
