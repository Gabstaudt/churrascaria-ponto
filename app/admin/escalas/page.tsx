import { CalendarCog, CalendarDays, ChevronLeft, ChevronRight, Clock3, Filter, Plus, UserRoundX } from "lucide-react";
...
const situationLabels = { WORK: "Trabalho", OFF: "Folga", VACATION: "Férias", LEAVE: "Afastamento", NO_SCHEDULE: "Sem jornada" } as const;
const situationOptions = Object.entries(situationLabels) as Array<[CalendarSituation, string]>;
...
export default async function ScheduleCalendarPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const situation = situationOptions.some(([value]) => value === query.situation) ? query.situation as CalendarSituation : undefined;
  ...
  return <main className="dashboard-page calendar-page">
    <div className="page-heading employees-heading"><div><p className="eyebrow">Planejamento operacional</p><h1>Escalas</h1><p>Consulte a previsão válida e registre ajustes específicos por data.</p></div><div className="page-heading-actions"><Link className="secondary-button detail-action" href="/admin/disponibilidade"><UserRoundX size={18} /> Folgas e ausências</Link><Link className="primary-button action-button" href="/admin/escalas/ajuste"><Plus size={18} /> Novo ajuste</Link></div></div>
    {query.saved === "1" ? <p className="success-message">Ajuste da escala salvo com sucesso.</p> : null}
    <section className="data-panel calendar-panel">
      <div className="calendar-controls"><div className="calendar-view-tabs" aria-label="Modo de visualização">{(["day", "week", "month"] as View[]).map((item) => <Link className={view === item ? "active" : ""} href={url({ ...baseQuery, view: item, date })} key={item}>{item === "day" ? "Dia" : item === "week" ? "Semana" : "Mês"}</Link>)}</div><div className="calendar-period"><Link aria-label="Período anterior" href={url({ ...baseQuery, date: move(date, view, -1) })}><ChevronLeft size={19} /></Link><div><CalendarDays size={18} /><strong>{periodTitle}</strong></div><Link aria-label="Próximo período" href={url({ ...baseQuery, date: move(date, view, 1) })}><ChevronRight size={19} /></Link></div></div>
      <form className="calendar-filters"><span><Filter size={17} /> Filtros</span><select name="employeeId" defaultValue={employeeId ?? ""}><option value="">Todos os funcionários</option>{filters.employees.map((employee) => <option value={employee.id} key={employee.id}>{employee.fullName}</option>)}</select><select name="position" defaultValue={position ?? ""}><option value="">Todos os cargos</option>{filters.positions.map((item) => <option value={item} key={item}>{item}</option>)}</select><select name="situation" defaultValue={situation ?? ""}><option value="">Todas as situações</option>{situationOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><input type="hidden" name="view" value={view} /><input type="hidden" name="date" value={date} /><button type="submit">Aplicar</button></form>
      <section className="calendar-results">{calendar.rows.length ? [...grouped.entries()].map(([day, rows]) => <article className="calendar-date" key={day}><header><div><strong>{new Intl.DateTimeFormat("pt-BR", { weekday: "long", timeZone: "UTC" }).format(new Date(`${day}T00:00:00Z`))}</strong><span>{formatDate(day)}</span></div><small>{rows.length} {rows.length === 1 ? "registro" : "registros"}</small></header><div>{rows.map((row) => <div className="calendar-entry" key={`${row.employee.id}-${day}`}><span className="calendar-person"><strong>{row.employee.fullName}</strong><small>{row.employee.position} · {row.employee.registrationNumber}</small></span><span className={`calendar-situation situation-${row.situation.toLowerCase()}`}>{situationLabels[row.situation]}</span><span className="calendar-hours"><Clock3 size={16} />{row.situation === "WORK" ? <strong>{row.startTime?.slice(0, 5) ?? "A definir"} — {row.endTime?.slice(0, 5) ?? "A definir"}</strong> : <strong>Sem horário</strong>}<small>{row.reason ?? row.scheduleName ?? "Nenhuma jornada vigente"}</small></span><Link className="calendar-adjust" href={`/admin/escalas/ajuste?employeeId=${row.employee.id}&date=${day}`}><CalendarCog size={16} /> Ajustar</Link></div>)}</div></article>) : <div className="empty-state"><CalendarDays size={30} /><h2>Nenhuma previsão encontrada</h2><p>Altere os filtros ou cadastre uma jornada para a equipe.</p></div>}</section>
    </section>
  </main>;
}
