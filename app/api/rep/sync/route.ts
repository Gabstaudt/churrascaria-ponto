...
import { PontoSyncREPAdapter } from "@/rep/ponto-sync-adapter";
import { repHeadersSchema } from "@/validations/rep";

export const dynamic = "force-dynamic"; export const runtime = "nodejs";
const MAX_BODY_BYTES = 1_000_000;
const adapters = { MOCK: new MockREPAdapter(), PONTO_SYNC_V1: new PontoSyncREPAdapter() } as const;
export async function POST(request: Request) {
  ...
  try { const text = await request.text(); if (Buffer.byteLength(text, "utf8") > MAX_BODY_BYTES) return Response.json({ error: "Lote excede 1 MB." }, { status: 413 }); const payload: unknown = JSON.parse(text); const adapter = adapters[device.adapter as keyof typeof adapters]; if (!adapter) return Response.json({ error: "Adapter do dispositivo não está disponível nesta versão." }, { status: 422 }); const batch = adapter.parse(payload); const result = await processRepBatch(device, headers.data.requestId, batch); log("info", "rep.batch_processed", { deviceId: device.id, syncId: result.syncId, replay: result.replay, received: result.received, inserted: result.inserted, duplicates: result.duplicates, rejected: result.rejected }); return Response.json(result, { status: 200, headers: { "Cache-Control": "no-store" } }); } catch (error) { if (error instanceof SyntaxError || error instanceof ZodError) return Response.json({ error: "Payload de sincronização inválido." }, { status: 400 }); log("error", "rep.batch_failed", { deviceId: device.id, requestId: headers.data.requestId, error }); return Response.json({ error: "Falha ao processar a sincronização." }, { status: 500 }); }
}
