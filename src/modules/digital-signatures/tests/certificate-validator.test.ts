...
describe("validação de certificado e mapeamento documental", () => {
  ...
  it("impede aplicar o padrão errado ao documento", () => {
    expect(() => assertSignatureMapping("AFD", "PADES")).toThrow("INVALID_SIGNATURE_MAPPING");
    expect(() => assertSignatureMapping("AEJ", "PADES")).toThrow("INVALID_SIGNATURE_MAPPING");
    expect(() => assertSignatureMapping("POINT_RECEIPT", "CADES")).toThrow("INVALID_SIGNATURE_MAPPING");
    expect(() => assertSignatureMapping("AFD", "CADES")).not.toThrow();
    expect(() => assertSignatureMapping("AEJ", "CADES")).not.toThrow();
    expect(() => assertSignatureMapping("POINT_RECEIPT", "PADES")).not.toThrow();
  });
});
